...
# ╔═╡ be536b1c-b128-4a13-ab2d-2a7c36db9549
using BenchmarkTools

# ╔═╡ 87b156b5-2c4f-4e06-98c8-4f0b2ed39187
using Roots

# ╔═╡ b17111ff-a88d-444c-a2e5-71ecd03d7522
using StaticArrays

# ╔═╡ b6eb14c1-e64e-4d43-92af-5d19bde011a3
using ForwardDiff

# ╔═╡ d0b0ff25-5af4-4035-ac86-85d8432eac0a
...
default(dpi = 180, linewidth = 2.5, label = false, background_color = :white)
...
Firms sell goods in a competitive market with $p(q) = p_0 - \delta q$. We assume energy is priced at marginal tax costs, such that output quantity is given by $q(a_t, \tau_t) = \frac{p_0 - \tau_t e_0 (1 - a)}{\delta}$. This yields a deadweight tax loss of 

$\begin{equation}
	l(a_t, \tau_t) \coloneqq \frac{\tau_t^2 e_0^2 (1 - a)^2}{2\delta}.
\end{equation}$
"
...
# ╔═╡ 24bfd059-658f-4f5a-be5d-87ef550b5886
function l(a, τ, firm::Firm)
	(τ * e(a, firm))^2 / firm.δ
end;
...
$\begin{equation}
    w(\tau_t, \bar{a}_t) \coloneqq y_0 \; d(\bar{a}_t) + c\left(\bar{a}_t\right) + l(\tau_t, a_t).
\end{equation}$
"
...
w(τ, a, government::Government, firm::Firm) = government.y₀ * d(e(a, firm), government, firm) + c(a, firm) + l(a, τ, firm);
...
let
	...
	plot!(A, a -> government.y₀ * d(e(a, firm), government, firm); c = :darkgreen, label = L"d(a) y_0")
...
let
	contourf(A, τspace, (a, τ) -> w(τ * taxfactor, a, government, firm); xticks = (aticks, aticklabels), yticks = (τticks, τticklabels), xflip = false, c = :Reds, linewidth = 0.5, xlabel = alabel, ylabel = τlabel, title = L"Social costs $w(\tau, a) \; \textrm{tUSD} / \textrm{year}$", clims = (0, Inf))
end
...
Under comittment $\tau_t = \tau^{\mathrm{c}}$, the firm takes $\tau^{\mathrm{c}}$ as given and chooses $a \in [0, 1]$ to minimise $k(a, \tau^c)$. The minimiser is given by $a(\tau^\mathrm{c}) = \min\left\{\frac{e_0}{\nu} \tau, 1\right\}$.

Then, the problem of a committed government amounts to minimising $w^{\mathrm{c}}(\tau) = w(\tau^{\mathrm{c}}, a^{\mathrm{c}}(\tau^\mathrm{c})))$.

"

# ╔═╡ 03385be6-b623-4e24-b0c0-eca701d37e77
function abatement(τ, firm::Firm)
	(firm.e₀ / firm.ν) * τ
end;

# ╔═╡ 4f618e04-c696-4bf8-8401-8a5ecbe0aa2d
wᶜ(τ, government, firm) = w(τ, abatement(τ, firm), government, firm);

# ╔═╡ 846a0839-a3e1-4288-af53-5d20915cf73f
begin
	const τᶜ = Optim.optimize(τ -> wᶜ(τ, government, firm), 0., maximum(τspace) * taxfactor).minimizer
	const committedwelfare = wᶜ(τᶜ, government, firm)
end;
...
let
	...
	plot!(twinx(fig), τspace, τ -> abatement(τ * taxfactor, firm), xlims = extrema(τspace), ylims = (0, Inf), c = :darkgreen, ylabel = alabel, yguidefontcolor = :darkgreen)
end
...
# ╔═╡ dfcdddee-dd17-45cc-b4fc-3bf7b729fdaf
τ₀ = 3 * taxfactor;
...
$\begin{equation}
	\tau_t = \frac{\tau^c + \alpha \bar{a}_t}{2 + \frac{e_0^2 (1 - a)^2}{2\delta \left(\frac{\epsilon}{\sigma}\right)^2 z}}.
\end{equation}$
"
...
# ╠═╡ skip_as_script = true
#=╠═╡
function optimalτ(a, z, signal::Signal, government::Government, firm::Firm)
	@unpack α, ϵ, σ = signal
	@unpack δ = firm
...
# ╔═╡ 838d74a7-701d-42ce-bc7a-a796920b277c
zmax = 50_000;

# ╔═╡ 16c84394-1525-409a-bb58-215756926056
md"
- ``a =`` $(@bind La Slider(A, show_value = true, default = 0.8))
- ``z =`` $(@bind Lz Slider(range(0, zmax, 201), show_value = true, default = 0.))
"
...
let
	...
	minimizer = optimalτ(La, Lz, signal, government, firm)
...
#=╠═╡
let
	zspace = range(0., zmax; length = 101)
	τ̄ = optimalτ(La, Inf, signal, government, firm)
	
	fig = plot(xlabel = L"Reputation weight $z$", ylabel = τlabel, ylims = (0, 1.01τ̄ / taxfactor))
	plot!(fig, zspace, z -> optimalτ(La, z, signal, government, firm) / taxfactor; c = :black)
...
let
	...
	fig = plot(xlabel = alabel, xticks = (aticks, aticklabels), ylims = (0, 13))
	...
	minimizer = abatement((Kτ * Kϕ + τᶜ * (1 - Kϕ)) * taxfactor, firm)
...
# ╔═╡ fb32684f-640e-42da-8ec7-5fb84cf703df
#=╠═╡
function equilibriumequation(τ, z, φ, signal::Signal, government::Government, firm::Firm)
	τᵉ = φ * τᶜ + (1 - φ) * τ
	optimalτ(abatement(τᵉ, firm), z, signal, government, firm) - τ
end;
  ╠═╡ =#

# ╔═╡ c6f6479d-1720-486e-adf4-9a5691c25c57
function equilibriumequation(τ, z, φ, model)
	signal, government, firm = model
	return equilibriumequation(τ, z, φ, signal, government, firm)
end;

# ╔═╡ 8eab14f7-fc3e-4e79-a49e-90e8397f2681
#=╠═╡
function equilibriumτ(z, φ, signal, government, firm; atol = 1e-5)
	model = (signal, government, firm)

	try
		find_zero(τ -> equilibriumequation(τ, z, φ, model), (zero(z), 1.5one(z)); atol = atol)
	catch exception 
		@warn "Failed with z = $z and φ = $φ"
		throw(exception)
	end
end;
  ╠═╡ =#

# ╔═╡ 66e944c1-8a20-4cb1-9d56-c53d0bfaecd4
#=╠═╡
let
	zspace = range(0, zmax, 1001)

	fig = plot(xlabel = L"Reputation weight $z$", ylabel = L"Equilibrium tax $\tau^*$", c = :black, linewidth = 2.5, xlims = (0., Inf), ylims = (0., Inf), legendtitle = L"\varphi", legend = :topright)
	
	φspace = 0:0.2:0.8
	cmap = palette([:darkorange, :darkblue], length(φspace))
	
	for (i, φ) ∈ enumerate(φspace)
		plot!(fig, zspace, z -> equilibriumτ(z, φ, signal, government, firm) / taxfactor; label = φ, c = cmap[i])
	end

	hline!(fig, [equilibriumτ(Inf, 0.5, signal, government, firm) / taxfactor]; c = :black)
	hline!(fig, [τᶜ / taxfactor]; c = :black, linestyle = :dash)

	fig
end
  ╠═╡ =#

# ╔═╡ f9f4bd61-6385-4be7-8531-073ab501ed74
md"
## Value Function
"

# ╔═╡ a569b3c7-6925-40e9-a0be-ed74331a06d7
#=╠═╡
function F(x, model, φ)
	...
	τ = equilibriumτ(z, φ, signal, government, firm)
	τᵉ = φ * τᶜ + (1 - φ) * τ
	a = abatement(τᵉ, firm)
	wᵒ = w(τ, a, government, firm)
	
	du = government.r * z
	dz = z + 2 * (wᵒ - u) * (σ / (ϵ * (τ - τᶜ)))^2

	return SVector{2}(du, dz) / (φ * (1 - φ))
end;
  ╠═╡ =#

# ╔═╡ a4d7e8e1-5f96-44b1-8a34-9aea090229ca
function condition(x, φ, integrator)
	u, z = x
    uinner = (u - 0.8wᶜ(τᶜ, government, firm)) * (u - 1.2w(0., 0., government, firm))
	zinner = z * (z - 10_000)

	return uinner * zinner
end;

# ╔═╡ aa8101bd-16f0-40f6-a4f9-44db6e5bd8cf
uspace = (wᶜ(τᶜ, government, firm), w(0., 0., government, firm));

# ╔═╡ 3fc8a1f7-2cb1-4d33-8cf3-09e7b2281ec4
md"
- ``u_0 =`` $(@bind u₀ Slider(range(uspace..., 101), show_value = true, default = uspace[1]))
- ``z_0 =`` $(@bind z₀ Slider(0.001:0.001:0.01, show_value = true, default = 0))
"

# ╔═╡ 95a523b3-ed06-43d7-89e5-7afb82a2d758
cb = ContinuousCallback(condition, terminate!);

# ╔═╡ 2186ac6e-d694-415f-93ff-cf02e213793a
#=╠═╡
begin
	x = SVector(u₀, z₀)
	ε = 1e-3
	prob = ODEProblem{false}(F, x, (ε, 1 - ε), (signal, government, firm))
end
  ╠═╡ =#

# ╔═╡ 58d13f72-90d3-4786-a60e-8192869e073f
#=╠═╡
let
	sol = solve(prob; callback = cb)

	plot(sol; idxs = 1, c = :darkred, xlims = (0, 1))
	plot!(twinx(), sol; idxs = 2, c = :darkblue, xlims = (0, 1))
end
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "8973d6a1a75f6a98f855e3e2f4f30de916c51c33"
...
git-tree-sha1 = "e05f8e2597c172aab2d88a61456dd01dc092e1b1"
...
git-tree-sha1 = "f2c5ddf74a49c1fd74e511d31168f0354c6c1e19"
