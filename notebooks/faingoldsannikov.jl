...
# ╔═╡ 3ece0a0f-063d-47cd-9e71-137c8cbc7a68
...
begin
	...
	A = range(0, e₀, 101)
	aticks = A
	alabel = L"Abatement $a$ [GtCO₂ / year]"
end;
...
$\begin{equation}
    k(a_{i, t}, \tau_t) = \underbrace{(e_{0} - a_{i, t})}_{e_{i, t}} \tau_t + c(a_{i, t}),
\end{equation}$ where $c(a) = \nu \frac{a^2}{2}.$

Aggregate emissions are given by $e_t = \int_0^\infty e_{i, t} \mathrm{d}i = e_0 - a_t$ where $a_t = \int_0^\infty a_{i, t} \mathrm{d}i.$
"

# ╔═╡ a7357976-2f2c-497f-9ab0-bdd9a809df1a
Base.@kwdef struct Firm{T}
	e₀::T = e₀ # emissions [GtC/year]
	ν::T = dietzφ * y₀ * CtoCO₂^2 # adjustment costs factor[(tEur / year) / (GtCO2 / year)^2]
end
...
Base.@kwdef struct Government{T}
	ξ::T = dicescc / (e₀ * CtoCO₂) # linear damage coefficient [-]
...
let
	fig = plot(xlabel = alabel, ylabel = L"Damages $\mathrm{tUSD /year}$", ylims = (0, Inf))
	plot!(fig, A, a -> government.y₀ * d(e(a, firm), government, firm); c = :darkgreen, label = L"d(a) y_0")
	plot!(A, a -> c(a, firm); c = :darkred, label = L"c(a)")
	plot!(A, a -> government.y₀ * l(τ₀, a, government, firm); c = :darkorange, label = L"l(\tau, a) y_0")
	
end

# ╔═╡ c7dfd8c7-1b38-4fc1-8162-0472e7a090ce
let
	contourf(A, τspace, (a, τ) -> w(τ * taxfactor, a, government, firm); yticks = (τticks, τticklabels), xflip = false, c = :Reds, linewidth = 0.5, xlabel = alabel, ylabel = τlabel, title = L"Social costs $w(\tau, a) \; \textrm{tUSD} / \textrm{year}$", clims = (0, Inf))
end
...
Under comittment $\tau_t = \tau^{\mathrm{c}}$, the firm takes $\tau^{\mathrm{c}}$ as given and chooses $a \in [0, e_0]$ to minimise $k(a, \tau^c)$. The minimiser is given by $a^{\mathrm{c}}(\tau^\mathrm{c}) = \frac{\tau^{\mathrm{c}}}{\nu} \land e_0$.

Then, the problem of a committed government amounts to find $\tau^c$ which minimises $w^{\mathrm{c}}(\tau) \coloneqq w(\tau, a^{\mathrm{c}}(\tau)))$.
"
...
begin
	const committedoptimisation = Optim.optimize(τ -> w(τ, aᶜ(τ, firm), government, firm), 0, firm.ν * firm.e₀)
	
	const τᶜ = Optim.minimizer(committedoptimisation)
	const wᶜ = Optim.minimum(committedoptimisation)
end;

# ╔═╡ 737e6fad-e711-4a52-bbae-3c9f66830573
let
	fig = plot(τspace, τ -> w(τ * taxfactor, aᶜ(τ * taxfactor, firm), government, firm), xlims = extrema(τspace), c = :black, xlabel = τlabel, xticks = (τticks, τticklabels), ylabel = L"Committed social costs $w^{\mathrm{c}}(\tau) \; \textrm{tUSD} / \textrm{year}$", yguidefontcolor = :black)
	
	vline!(fig, [τᶜ / taxfactor]; c = :black, linestyle = :dash)
	
	plot!(twinx(fig), τspace, τ -> aᶜ(τ * taxfactor, firm), xlims = extrema(τspace), ylims = (0, 1.05e₀), c = :darkgreen, ylabel = alabel, yguidefontcolor = :darkgreen)
end
...
We assume that the signal $s_t$ is growing in the policy rate $\tau$, that is $\mu(\tau, a) \coloneqq \epsilon \tau$.
...
Introduce the reputational capital $z \geq 0$. The optimal tax policy $\tau$, given an abatement level $a$ and the reputational weight $z$ is defined as

$\begin{equation}
	\begin{split}
		\tau \in \arg_{\tau}\min \; &\mathcal{L}(\tau; a, z) \text{ where } \\
		&\mathcal{L}(\tau; a, z) \coloneqq w(\tau, a) - z \frac{ \mu(\tau^{\mathrm{c}}, a) - \mu(\tau, a)}{\sigma^2} \mu(\tau, a).
	\end{split}
\end{equation}$ 

First order condition yields the optimal policy $\tau_t = \eta(z_t, a_t) \tau^c$ where 

$\begin{equation}
	\eta(a, z) \coloneqq \frac{1}{2 + \frac{\delta (e_0 - a)^2}{y_0 z \left(\frac{\epsilon}{\sigma}\right)^2}}
\end{equation}$
"
...
md"
- ``a =`` $(@bind La Slider(A, show_value = true, default = 0.))
...
let
	...
	fig = plot(xlabel = τlabel, xticks = (τticks, τticklabels), legendtitle = L"Reputation $z$", legendtitlefontsize = 9, legendfontsize = 9, ylabel = L"Welfare $\mathcal{L}(\tau; %$(round(La, digits = 2)), z) \textrm{tUSD} / \textrm{year}$", legend = :topleft)
	
	plot!(fig, τspace, τ -> L(τ * taxfactor, La, zmin, signal, government, firm), label = L"%$zmin", c = cmin)

	cweight = (Lz - zmin) / (zmax - zmin)
	c = get(cgrad([cmin, cmax]), cweight)

	minimizer = η(La, Lz, signal, government, firm) * τᶜ
	minimum = L(minimizer, La, Lz, signal, government, firm)
			
	plot!(fig, τspace, τ -> L(τ * taxfactor, La, Lz, signal, government, firm), label = L"%$Lz", c = c)
	...
	plot!(fig, τspace, τ -> L(τ * taxfactor, La, zmax, signal, government, firm), label = L"%$zmax", c = cmax)
...
let
	...
	τ̄ = τᶜ * η(La, Inf, signal, government, firm)
	
	fig = plot(xlabel = L"Reputation weight $z$", ylabel = τlabel, ylims = (0, 1.01τ̄ / taxfactor), xlims = (0, Inf))
	plot!(fig, zspace, z -> τᶜ * η(La, z, signal, government, firm) / taxfactor; c = :black)
...
let
	...
	fig = plot(xlabel = alabel, ylims = (0, Inf))
	plot!(fig, A, a -> K(a, Kτ * taxfactor, 0., firm); label = L"0 \%", c = cmin, ylabel = L"Costs $k^{\phi}(\tau; %$(round(La, digits = 2)), z) \textrm{tUSD} / \textrm{year}$")
	...
	annotate!(fig, minimizer, minimum, text(L"a = %$(round(minimizer, digits = 2))", 10, :bottom))
	
	plot!(fig, A, a -> K(a, Kτ * taxfactor, 1., firm); label = L"100 \%", c = cmax)
	
end

# ╔═╡ b36f2afd-c3a2-45c7-b9c8-5fcc485965f3
md"
### Equilibrium

Given the government best response, the equilibrium abatemnet level, given beliefs $\phi$ and reputational capital $z$ is given by

$\begin{equation}
	a^* = \frac{\tau^c}{\nu} \big( \phi + (1 - \phi) \; \eta(z, a^*) \big) \land e_0.
\end{equation}$
"

# ╔═╡ df7fad36-6047-4547-809a-f4c342c93b17
function abatementequilibrium(a, ϕ, z, signal, government, firm)
	(τᶜ / firm.ν) * (ϕ + (1 - ϕ) * η(a, z, signal, government, firm)) - a
end;

# ╔═╡ 84cce12b-2107-4e60-9ac4-00496d10dfae
function aᵉ(ϕ, z, signal, government, firm)
	bounds = (zero(firm.e₀), firm.e₀)
	
	lower = abatementequilibrium(bounds[1], ϕ, z, signal, government, firm)
	upper = abatementequilibrium(bounds[2], ϕ, z, signal, government, firm)

	if upper ≥ 0
		return 0.
	else
		return find_zero(a -> abatementequilibrium(a, ϕ, z, signal, government, firm), bounds)
	end
end;
...
let
	...
	contourf(ϕs, zspace, (ϕ, z) -> e(aᵉ(ϕ, z, signal, government, firm), firm); xlabel = L"Reputation $\phi$", ylabel = L"Reputation capital $z$", title = L"Equilibrium emissions $a^*(\phi, z)$", c = :Reds, linewidth = 1., size = 600 .* (√2, 1), clims = (0, Inf), levels = 51)
end

# ╔═╡ a0dea2de-084a-4c9b-95b9-e789d03b6c52
md"## Value function"

# ╔═╡ 66eac0c5-3178-4a67-887d-8718427d4dda
function F!(dx, x, parameters, ϕ)
	signal, government, firm = parameters
	u, z = x

	@unpack σ, ϵ = signal
	@unpack r = government

	a = aᵉ(ϕ, z, signal, government, firm)
	τ = ifelse(z > 0, η(a, z, signal, government, firm) * τᶜ, zero(τᶜ))
...
# ╔═╡ fdb58a92-2383-4094-b600-184286dac605
begin
	εϕ = 1e-5
	ϕspan = (εϕ, 1 - εϕ)
	const u₀ = w(0, 0, government, firm)
	const u₁ = wᶜ
	
	function bca!(resid, u, parameters)
		signal, government, firm = parameters
	    resid[1] = u[1] - u₀
	end
	
	function bcb!(resid, u, parameters)
		signal, government, firm = parameters
	    resid[1] = u[1] - u₁
	end
	
	const Δu = (u₁ - u₀) / (ϕspan[2] - ϕspan[1])
	
	function guess(phi)
	    û = u₁ + Δu * (phi - εϕ)
	    ẑ = -Δu * phi * (1 - phi) / government.r
	    return [û, ẑ]
	end
	
	prob = TwoPointBVProblem(F!, (bca!, bcb!), guess, ϕspan; bcresid_prototype = (zeros(1), zeros(1)))
	
	sol = solve(prob, MIRK4(); dt = 1e-3, abstol = 1e-8, reltol = 1e-8)
...
manifest_format = "2.0"
project_hash = "7e0e55616bc3d07f37056802d871848cc00a54e7"
...
git-tree-sha1 = "d2c9cda8321a1bb1e13b1c6694792d053cae0d59"
...
git-tree-sha1 = "76710fe77065e0a29f47f91cd8f8038dd6a8a4cc"
...
git-tree-sha1 = "776af861ee6613f70eb656f04683e624bc391d83"
...
git-tree-sha1 = "b3659387899568593bd67ae36eaaae4353ce41fa"
...
git-tree-sha1 = "4234288ab0b141be4551dcf0d1e2e526051c3288"
...
git-tree-sha1 = "49fb590f976f5c93dd2b574b636ab921745905f6"
...
git-tree-sha1 = "ca7d90cc2003a9bf0e502745f4e8f9a28727da33"
...
git-tree-sha1 = "cc9de443eec2be77a639c67432221f93e6f3c28e"
...
git-tree-sha1 = "803334224154e4a2a9d984547dd4449d3ca71c63"
...
git-tree-sha1 = "8df990b85ea063a20df3f1b1dddab9d0afec985c"
...
deps = ["DiffEqBase", "FastBroadcast", "LinearAlgebra", "MuladdMacro", "OrdinaryDiffEqCore", "PrecompileTools", "Preferences", "RecursiveArrayTools", "Reexport", "SciMLBase", "TruncatedStacktraces"]
git-tree-sha1 = "fb8ae6a290beed2ef61fa2e7571d921e1100c365"
