...
# ╔═╡ 1e7a1b04-d0b1-4cb8-8e86-0049c2814c6d
using ForwardDiff

# ╔═╡ e28cbe60-6e4f-4e5d-ae0a-f66cfb512808
using StaticArrays
...
function c(ϕ, firm::Firm)
	firm.κ * ϕ + firm.ν * ϕ^2 / 2
end;
...
function k(ϕ, a, τ, firm::Firm)
	e(a, firm) * τ + c(ϕ, firm)
end;

# ╔═╡ edccbe86-1a4f-494b-ac7c-2e3c916817f8
...
md"
# Uncomitted Government
"

# ╔═╡ a4eb2917-e3e9-4be2-b857-573cdcb81eb1
md"
## $t = 1$
### Firm
...
# ╔═╡ d58e3ba4-a4ef-4e91-94d1-b4278dc4cf35
function Π₁(ϕ₁, τ₂, a₁, firm::Firm)
	c(ϕ₁, firm) + firm.β * τ₂ * e(f(a₁, ϕ₁, firm), firm)
end;

# ╔═╡ 96e8ccd0-ff86-435a-9538-1f4ef7ff49cf
function Π₁′(ϕ₁, τ₂, a₁, firm::Firm)
	ForwardDiff.derivative(ϕ -> Π₁(ϕ, τ₂, a₁, firm), ϕ₁)
end;

# ╔═╡ 2057aea6-c8fb-498f-bac0-c7c8a81c88db
function Φ₁(τ₂::T, a₁, firm::Firm; ϕ̄ = 50) where T <: Real
	...
	if Π₁′(l, τ₂, a₁, firm) ≥ 0
		return l
	elseif Π₁′(u, τ₂, a₁, firm) ≤ 0
		return u
	else
		find_zero(ϕ₁ -> Π₁′(ϕ₁, τ₂, a₁, firm), (l, u))
	end
end;

# ╔═╡ 4d8ec968-92c3-483a-8fb4-78844e3c56de
md"
### Government
"

# ╔═╡ 3a977410-5e08-48cf-8047-5bae7be6611e
function w₂(τ₂, a₁, gov::Gov, firm::Firm)
	ϕ₁ = Φ₁(τ₂, a₁, firm)
	a₂ = f(a₁, ϕ₁, firm)
	e₂ = e(a₂, firm)
	
	return c(ϕ₁, firm) + gov.β * gov.y₀ * d(e₂, gov)
end;

# ╔═╡ f5fb9b67-2086-4566-897f-f3725bf1cc0a
function w₂′(τ₂, a₁, gov::Gov, firm::Firm)
	ForwardDiff.derivative(τ -> w₂(τ, a₁, gov, firm), τ₂)
end;

# ╔═╡ a1450c5f-fb01-4eac-9fde-8a7904373ed9
function Θ₂(a₁::T, gov::Gov, firm::Firm; τ̄ = 50) where T <: Real
	l = -one(T)
	u = τ̄ * one(T)
	
	if w₂′(l, a₁, gov, firm) ≥ 0
		return l
	elseif w₂′(u, a₁, gov, firm) ≤ 0
		return u
	else
		find_zero(τ -> w₂′(τ, a₁, gov, firm), (l, u))
	end
end;

# ╔═╡ 199553fe-1b0b-4a32-9c41-125b763112e1
md"## $t = 0$"

# ╔═╡ c67eeb80-3e6e-4e68-8be0-52cec7052d5c
md"### Firm"

# ╔═╡ fa47974e-6474-474f-9268-5bd00f8ea294
function Π₀(ϕ₀, τ₁, a₀, gov::Gov, firm::Firm)
	@unpack β, δ = firm
	...
	c(ϕ₀, firm) + β * τ₁ * e(a₁, firm) + 
		β * Π₁(ϕ₁, τ₂, a₁, firm)
end;

# ╔═╡ c945d3b2-dea3-4aac-8956-55f3ec74adf3
function Π₀′(ϕ₀, τ₁, a₀, gov::Gov, firm::Firm)
	ForwardDiff.derivative(ϕ -> Π₀(ϕ, τ₁, a₀, gov, firm), ϕ₀)
end;

# ╔═╡ fcd3c217-e416-4f9c-8354-69442590990e
function Φ₀(τ₁::T, a₀, gov::Gov, firm::Firm; ϕ̄ = 50) where T <: Real
	l = -one(T)
	u = ϕ̄ * one(T)
	
	if Π₀′(l, τ₁, a₀, gov, firm) ≥ 0
		return l
	elseif Π₀′(u, τ₁, a₀, gov, firm) ≤ 0
		return u
	else
		find_zero(ϕ₀ -> Π₀′(ϕ₀, τ₁, a₀, gov, firm), (l, u))
	end
end;

# ╔═╡ bbeaa7c7-5f3b-45ac-8581-023f335328ea
md"### Government"

# ╔═╡ ca678bc7-5ec4-4d1d-9fe3-fa3ef009c295
function w₁(τ₁, a₀, gov::Gov, firm::Firm)
	@unpack δ = firm
	@unpack β, y₀ = gov
	...
	c(ϕ₀, firm) + β * y₀ * d(e₁, gov) +
		β * w₂(τ₂, a₁, gov, firm)
end;

# ╔═╡ dc7715b3-c9fd-461e-b239-86614def06bf
function w₁′(τ₁, a₀, gov::Gov, firm::Firm)
	ForwardDiff.derivative(τ -> w₁(τ, a₀, gov, firm), τ₁)
end;

# ╔═╡ 532be8a8-f4be-477d-b7e5-f80642caf7a3
function Θ₁(a₀::T, gov::Gov, firm::Firm; τ̄ = 50) where T <: Real
	l = -one(T)
	u = τ̄ * one(T)
...
# ╔═╡ 3c949d9c-c9a9-4c03-b6e5-4f842e8224cf
md"# Committed Government"

# ╔═╡ 94c7466a-aa5e-4790-8223-67745debf94b
function Πᶜ(ϕ, τ, a₀, gov::Gov, firm::Firm)
	@unpack β = firm
	ϕ₀, ϕ₁ = ϕ
	τ₁, τ₂ = τ
	...
	c(ϕ₀, firm) + β * τ₁ * e(a₁, firm) +
		β * c(ϕ₁, firm) + β^2 * τ₂ * e(a₂, firm)
end;

# ╔═╡ 63f041d1-78c9-49ba-93c7-0519971c7b23
function Πᶜ(ϕ, params)
	τ, a₀, gov, firm = params
	return Πᶜ(ϕ, τ, a₀, gov, firm)
end;

# ╔═╡ 6bfa13eb-5b6e-41c7-8f54-bb4ec3ad9dd3
function Φᶜ(τ, a₀::T, gov::Gov, firm::Firm) where T
	params = (τ, a₀, gov, firm)
	obj = OptimizationFunction{false}(Πᶜ, AutoForwardDiff());
	prob = OptimizationProblem(obj, MVector{2, T}(0, 0), params)
...
# ╔═╡ c9061150-76e5-46e4-ad80-3db488487f08
function wᶜ(τ, a₀, gov::Gov, firm::Firm)
	@unpack β = gov
	...
	c(ϕ₀, firm) + β * y₀ * d(e₁, gov) + β * c(ϕ₁, firm) + β^2 * y₀ * d(e₂, gov)
end;

# ╔═╡ fb2f1aa8-2177-4a13-a912-716f9510e2fa
function wᶜ(τ, params)
	a₀, gov, firm = params
	return wᶜ(τ, a₀, gov, firm)
end;

# ╔═╡ feeb504d-66fc-4e07-b182-124755df08c6
function Θᶜ(a₀::T, gov, firm) where T
	params = (a₀, gov, firm)
	obj = OptimizationFunction{false}(wᶜ, AutoForwardDiff());
	prob = OptimizationProblem(obj, MVector{2, T}(0, 0), params)
...
# ╔═╡ 6c9b1da7-d0a7-47db-b9d7-adbbff7cfb6c
md"# Comparison"

# ╔═╡ 6246430d-74af-4036-a363-b2f6338ab16c
function Θ(a₀, gov::Gov, firm::Firm)
	τ₁ = Θ₁(a₀, gov, firm)
	ϕ₀ = Φ₀(τ₁, a₀, gov, firm)
...
# ╔═╡ adc4c703-bc28-44b6-887e-ab0ba518bdf8
let

	committed = [Θᶜ(a, gov, firm) for a in A]

	committedfig = plot(xlabel = L"Abatement $a_0 \; \mathrm{GtCO}_2 / \mathrm{year}$", ylabel = L"Initial tax $\tau_1 \: \mathrm{USD} / \mathrm{tCO}_2$", title = "Committed", ylims = (0, 12))
	plot!(committedfig, A, first.(committed); c = :darkred)
	plot!(committedfig, A, last.(committed); c = :darkblue)
	
	uncommitted = [Θ(a, gov, firm) for a in A]
	uncommittedfig = plot(xlabel = L"Abatement $a_0 \; \mathrm{GtCO}_2 / \mathrm{year}$", title = "Uncommitted", ylims = (0, 12), yticks = nothing)
	plot!(uncommittedfig, A, first.(uncommitted); label = L"\tau_1", c = :darkred)
	plot!(uncommittedfig, A, last.(uncommitted); label = L"\tau_2", c = :darkblue)


	plot(committedfig, uncommittedfig, size = 400 .* (2√2, 1), link = :y, left_margin=[6Plots.mm -7Plots.mm], bottom_margin = 5Plots.mm)
end

# ╔═╡ 3ed3295c-df20-480e-9ef2-e893ef6cbf1e
let
	diff = [Θᶜ(a, gov, firm) .- Θ(a, gov, firm) for a in A]

	difffig = plot(xlabel = L"Abatement $a_0 \; \mathrm{GtCO}_2 / \mathrm{year}$", ylabel = L"Initial tax $\tau_1 \: \mathrm{USD} / \mathrm{tCO}_2$")
	plot!(difffig, A, first.(diff); c = :darkred, label = L"\tau^c_1 - \tau_1")
	plot!(difffig, A, last.(diff); c = :darkblue, label = L"\tau^c_2 - \tau_2")
	hline!([0.], c = :black)
end
...
manifest_format = "2.0"
project_hash = "34a4da6dda4893f439f57ec809758d4301e7deea"
