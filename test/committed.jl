...
for θ in θspace
    ...
    let
        ...
        investmentfig = contourf(1:T, A, valuefunction.Φ; xlabel = L"t", ylabel = L"a", title = L"Investment $\phi_t(a)$", camera = (15, 30), opacity = 1, xlims = (1, T), ylims = extrema(A), linewidth = 0.5, clims = (0, Inf), c = :Greens)
...
let
    fig = plot(xlabel = L"$t$", margins = 5Plots.mm, ylabel = L"Investment $\phi_t$")
    colors = range(RGB(0.5, 0.5, 0.5), RGB(0, 1, 0); length = length(θspace))
    for (i, θ) in enumerate(θspace)
        plot!(fig, 1:(T-1), ϕtrajectories[θ]; label = L"$\theta = %$(θ)$", c = colors[i])
    end
    figpath = joinpath(plotpath, "committed", "paths"); if !ispath(figpath) mkpath(figpath) end
